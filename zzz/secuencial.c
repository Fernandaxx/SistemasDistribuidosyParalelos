...
void NQueens(void)
{
    ...
    SIZEE  = SIZE - 1;          // Último indice del tablero, es N-1 (E de End)
    BOARDE = &BOARD[SIZEE];     // Dirección de la última posicion del tablero
    TOPBIT = 1 << SIZEE;        // MSB en 1 y N-1 en 0
    MASK   = (1 << SIZE) - 1;   // N bits en 1

    /* fila 0:000000001 */
    /* fila 1:011111100 no puede probar en las últimas 2 pq se atacan, la primera no sé */
    BOARD[0] = 1; // Pone 00000001 en la fila 0
    for (BOUND1=2; BOUND1<SIZEE; BOUND1++) {
        BOARD[1] = bit = 1 << BOUND1; // bit = 00000001000 donde está la reina
        Backtrack1(2, (2 | bit)<<1, 1 | bit, bit>>1);
    }
    /* 0:000001110 */
    SIDEMASK = LASTMASK = TOPBIT | 1; // 10000001
    ENDBIT = TOPBIT >> 1; // 01000000
    for (BOUND1=1,BOUND2=SIZE-2; BOUND1<BOUND2; BOUND1++,BOUND2--) {
        BOARD1 = &BOARD[BOUND1];
        BOARD2 = &BOARD[BOUND2];
        BOARD[0] = bit = 1 << BOUND1;
        Backtrack2(1, bit<<1, bit, bit>>1);
        LASTMASK |= LASTMASK>>1 | LASTMASK<<1; // lastmask = lastmask | 01000010 = 11000011
        ENDBIT >>= 1;
    }
